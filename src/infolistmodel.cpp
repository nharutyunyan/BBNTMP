...
inline const static QStringList getVideoFileList() {
	...
	filters << "*.avi" << "*.mp4" << "*.3gp" << "*.3g2" << "*.asf" << "*.wmv"
			<< "*.mov" << "*.m4v" << "*.f4v"; //these are the formats the don't crash the app.
...
InfoListModel::InfoListModel(QObject* parent) :
		m_selectedIndex(QVariantList()), m_file(
				QDir::home().absoluteFilePath("videoInfoList.json")) {
	...
	QObject::connect(this, SIGNAL(finishedThumbnailGeneration()),
			m_producerThread, SLOT(quit()));
	...
	QObject::connect(reader, SIGNAL(metadataReady(const QVariantMap& )), this,
			SLOT(onMetadataReady(const QVariantMap& )));
	QObject::connect(this, SIGNAL(setData(QStringList )), reader,
			SLOT(setData(QStringList )));
	prepareToStart();
	getVideoFiles();
}
...
int InfoListModel::childCount(const QVariantList& indexPath) {
	int childCount = GroupDataModel::childCount(indexPath);

	qDebug() << "childCount " << childCount;

	if (childCount >= 4 && m_drillDown == false) {
		return 4;
	}
	return childCount;
}
...
void InfoListModel::getVideoFiles() {
	...
	for (QStringList::const_iterator i = result.begin(); i != result.end();
			++i) {
...
void InfoListModel::updateListWithAddedVideos(const QStringList& result) {
	...
	for (QStringList::const_iterator i = result.begin(); i != result.end();
		...
		if (!set.contains(*i)) {
			...
			QStringList pathElements = i->split('/', QString::SkipEmptyParts,
					Qt::CaseSensitive);
			...
			// Add folder
			val["folder"] = folderFieldName(
					pathElements[pathElements.size() - 2]);
...
void InfoListModel::load() {
	...
	if (jda.hasError()) {
		...
		qDebug() << m_file << "JSON loading error: " << error.errorType()
				<< ": " << error.errorMessage();
	} else {
...
void InfoListModel::saveData() {
	...
	if (jda.hasError()) {
		bb::data::DataAccessError error = jda.error();
		qDebug() << m_file << "JSON save error: " << error.errorType() << ": "
				<< error.errorMessage();
	} else {
...
void InfoListModel::onMetadataReady(const QVariantMap& val) {
	...
	infoMap["duration"] =
			val.value(bb::multimedia::MetaData::Duration).toString();
...
void InfoListModel::setValue(QVariantList ix, const QString& fld_name,
		const QVariant& val) {
...
QString InfoListModel::getFormattedTime(int msecs) {
	...
	int seconds = (msecs - (minutes * 1000 * 60) - (hours * 1000 * 60 * 60))
			/ 1000;
...
void InfoListModel::toggleFolder(QString folderName) {
	...
	QList<QVariantMap> parrentBuffer;
	for (int i = 0; i < m_currentSelectionList.size(); i++) {
		QVariantList index = m_currentSelectionList[i];
		QVariantMap v = data(index).toMap();
		...
		if (v["folder"] != folderName) {
			...
			QStringList pathElements = path.split('/', QString::SkipEmptyParts,
					Qt::CaseSensitive);
			v["folder"] = folderFieldName(
					pathElements[pathElements.size() - 2]);
		}
		// We remove the node because we will reinsert it once the buffer is full

		//remove item from group datamodel
		removeAt(index);
		//store movie to be moved
		folBuffer.push_front(v);

		// If there was really more items
		QVariantList parentIndex = index;
		//get the folder the movie was from
		parentIndex.removeLast();
		if (childCount(parentIndex) >= 4) {
			if (folBuffer.at(i).key("0Favorites") == "folder") {
				//add which item to add (in this case the 4th item since we only display 4)
				parentIndex.append(3);
				itemAdded(parentIndex);
			}
		}
	}
	...
	for (int i = 0; i < folBuffer.size(); i++) {
		...
		insert(folBuffer[i]);
		//get the folder and location inside it
		QVariantList input = find(folBuffer[i]);
		//remove location so we can get the child count of the folder and not the item
		input.removeLast();
		if(childCount(input) >= 4) {
			//if we have 4 items already remove the last one to prevent displaying of empty
			input.append(4);
			itemRemoved(input);
		}
	}
